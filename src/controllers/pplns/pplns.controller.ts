...
export class PplnsController {
    ...
    // Read directly from config, not from a coinbase - these values are
    // never written on-chain (see
    // doc-elektron/fix-report-pool-identity-utxo-attestation.md for why an
    // earlier revision that tried this was reverted). Used both as a
    // display convenience for elektron-net-ppool-ui and as the payload
    // this pool reports to mempool explorer instances (see
